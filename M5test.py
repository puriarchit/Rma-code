...
def ensure_indexes(cursor):
    ...
    if cursor.fetchone()[0] == 0:
        ...
        if cursor.fetchone()[0] > 0:
            logging.info("Source table NegativeList_New1 already consumed, NegativeList target exists. Proceeding with Master & Filter Sync...")
        else:
            raise RuntimeError(
...
def populate_master_and_filter(cursor, inserted_total):
    ...
    # Step G1: Drop existing target tables for SELECT INTO Minimal Logging Bulk Engine
    ...
    # Step G2: SELECT ... INTO Engine for NegativeList_Master (Minimal Logging, Extent Page Bulk Allocation)
    t_m_insert = time.time()
    logging.info("  [Fast SELECT INTO Engine Step 1] Bulk creating & populating NegativeList_Master via SELECT INTO...")
    cursor.execute(
        """
        SELECT
        ...
        INTO dbo.NegativeList_Master
        FROM dbo.NegativeList A WITH (NOLOCK);
        """
    )
    master_insert_sec = time.time() - t_m_insert
    logging.info("  ⚡ FAST ENGINE METRIC 1: NegativeList_Master SELECT INTO Bulk Sync: %.2f seconds (%.2f mins)!", master_insert_sec, master_insert_sec / 60)

    # Step G3: Post-Load Primary Key Nonclustered Build on NegativeList_Master
    t_m_pk = time.time()
    logging.info("  [Fast SELECT INTO Engine Step 2] Building Post-Load Primary Key (PK_NegativeList_Master NONCLUSTERED)...")
    ...
    logging.info("  ⚡ FAST ENGINE METRIC 2: NegativeList_Master Post-Load PK Build: %.2f seconds (%.2f mins)!", master_pk_sec, master_pk_sec / 60)

    # Step G4: SELECT ... INTO Engine for NegativeListFilter
    t_f_insert = time.time()
    logging.info("  [Fast SELECT INTO Engine Step 3] Bulk creating & populating NegativeListFilter via SELECT INTO...")
    cursor.execute(
        """
        SELECT
            i.ID,
            ...
            i.Nationality
        INTO dbo.NegativeListFilter
        FROM dbo.NegativeList i WITH (NOLOCK);
        """
    )
    filter_insert_sec = time.time() - t_f_insert
    logging.info("  ⚡ FAST ENGINE METRIC 3: NegativeListFilter SELECT INTO Bulk Sync: %.2f seconds (%.2f mins)!", filter_insert_sec, filter_insert_sec / 60)

    # Step G5: Post-Load Primary Key / Index Build on NegativeListFilter
    t_f_pk = time.time()
    logging.info("  [Fast SELECT INTO Engine Step 4] Building Post-Load Index / PK on NegativeListFilter...")
    try:
    ...
    logging.info("  ⚡ FAST ENGINE METRIC 4: NegativeListFilter Post-Load PK Build: %.2f seconds (%.2f mins)!", filter_pk_sec, filter_pk_sec / 60)
