...
# Optimization: Used LexisNexis_Data.dbo.Country reference lookup
cursor.execute("""
...
cursor.execute("""
    ...
    FROM Scanned s
    LEFT JOIN LexisNexis_Data.dbo.Country c ON s.ISOStandard = c.tISO
    WHERE s.cnt = 1
""")
conn.commit()
...
cursor.execute("""
    ...
    LEFT JOIN LexisNexis_Data.dbo.Country c ON d.ISOStandard = c.tISO
""")
conn.commit()
