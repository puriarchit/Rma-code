...
dt_nat = time.time() - t0
logs.append(("Lookup Table #TempNationalities", nat_count, dt_nat))
...
cursor.execute("SELECT COUNT(*) FROM #PEP_GUIDs")
pep_count = cursor.fetchone()[0]
dt_pep = time.time() - t0
logs.append(("Lookup Table #PEP_GUIDs", pep_count, dt_pep))
conn.commit()
...
print("--> Step 1/4: joining DOB & Address details...")
t0 = time.time()
...
logs.append(("Consolidation Step 1 (DOB/Address)", g1_count, dt))
print(f"    [Step 1 Complete] {g1_count:,} records processed in {dt:.2f} seconds.")

# Step 2: Join Remarks + Web Links (Narrow intermediate)
print("--> Step 2/4: joining Remarks & Web Links...")
t0 = time.time()
...
dt = time.time() - t0
master_total_time += dt
logs.append(("Consolidation Step 2 (Remarks/Links)", g2_count, dt))
print(f"    [Step 2 Complete] {g2_count:,} records processed in {dt:.2f} seconds.")

# Step 3: Join Enforcements + Sanctions + Citizenships (Narrow intermediate)
print("--> Step 3/4: joining Enforcements, Sanctions & Citizenships...")
t0 = time.time()
...
dt = time.time() - t0
master_total_time += dt
logs.append(("Consolidation Step 3 (Sanctions/Citizens)", g3_count, dt))
print(f"    [Step 3 Complete] {g3_count:,} records processed in {dt:.2f} seconds.")

# Step 4: Final insertion into NegativeList_New (Includes late projection of Entity, Identifications & Nationalities)
print("--> Step 4/4: performing final master profile compilation and insert...")
t0 = time.time()
cursor.execute("""
...
dt = time.time() - t0
master_total_time += dt
logs.append(("Consolidation Step 4 (Final Insert)", rows_master, dt))
print(f"    [Step 4 Complete] {rows_master:,} profiles written in {dt:.2f} seconds.")
...
split_total_time += dt
logs.append(("Splits: Non-PEP Watchlist", rows_nonpep, dt))
print(f"    Non-PEP Watchlist split complete: {rows_nonpep:,} records in {dt:.2f} seconds.")
...
split_total_time += dt
logs.append(("Splits: Sanctions/Enforcements", rows_sanc, dt))
print(f"    Sanctions/Enforcements split complete: {rows_sanc:,} records in {dt:.2f} seconds.")
...
split_total_time += dt
logs.append(("Splits: PEP Watchlist", rows_pep, dt))
print(f"    PEP Watchlist split complete: {rows_pep:,} records in {dt:.2f} seconds.")
...
print("\n" + "="*80)
print("                         CONSOLIDATION WORKFLOW REPORT")
print("="*80)
print(f"{'Step Name':<38} | {'Rows Affected':<13} | {'Time Taken (Sec)':<16}")
print("-"*80)
for step_name, r_count, s_time in logs:
    print(f"{step_name:<38} | {r_count:<13,} | {s_time:<16.2f}")
print("-"*80)
print(f"{'Total Master Records Consolidated':<38} | {rows_master:<13,} | {master_total_time:<16.2f} ({master_total_time/60:.2f} min)")
print(f"{'Total Watchlist Splits Categorized':<38} | {split_total_rows:<13,} | {split_total_time:<162.2f} ({split_total_time/60:.2f} min)" if split_total_time > 0 else f"{'Total Watchlist Splits Categorized':<38} | {split_total_rows:<13,} | {split_total_time:<16.2f} ({split_total_time/60:.2f} min)")
print("="*80 + "\n")
