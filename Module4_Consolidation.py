...
# 1. Load non-PEP profiles and consolidated watchlist matches directly into NegativeList_New1
cursor.execute("""
    ...
    OPTION (MERGE JOIN, HASH JOIN, MIN_GRANT_PERCENT = 20)
""")
conn.commit()
print(f"non-PEP and consolidated profiles loaded, took {time.time() - step_start:.2f} seconds.")

print("loading PEP category profiles...")
step_start = time.time()

# 2. Load PEP profiles directly into NegativeList_New1
cursor.execute("""
    INSERT INTO NegativeList_New1 WITH (TABLOCK) (
        EntityGUID, ReferenceID, EntityType, Gender, FirstName, LastName, SecondName, Title,
        DOB, ALTDOB1, ALTDOB2, ALTDOB3, AddressLine1, AddressLine2, City, Country, POB,
    ...
    LEFT JOIN #TempNationalities J ON A.EntityGUID = J.EntityGUID
    LEFT JOIN Entity_Citizenship_New K WITH (NOLOCK) ON A.EntityGUID = K.EntityGUID
    OPTION (MERGE JOIN, HASH JOIN, MIN_GRANT_PERCENT = 10)
""")
conn.commit()
