...
def main():
    ...
    cursor.execute("""
        ...
        POB_Data AS (
            SELECT EntityGUID, ISOStandard, MIN(City) AS POB
            FROM EntityAddress
            WHERE AddressTypeDesc = 'Place Of Birth'
              AND City IS NOT NULL
              AND City != ''
            GROUP BY EntityGUID, ISOStandard
        ),
        Country_Data AS (
        ...
        FROM AllAddresses a
        LEFT JOIN POB_Data p ON a.EntityGUID = p.EntityGUID AND a.CountryCode = p.ISOStandard
    ...
    cursor.execute("""
        ...
        SELECT EntityGUID, DOB, 
               ROW_NUMBER() OVER (PARTITION BY EntityGUID ORDER BY DOB ASC) as row_rank
        FROM (
            SELECT EntityGUID, DOB
            FROM (
                SELECT EntityGUID,
    ...
    cursor.execute("""
        ;WITH Scanned AS (
            ...
            WHERE IdentificationTypeDesc COLLATE SQL_Latin1_General_CP1_CS_AS LIKE 'National Id%'
        )
    ...
    cursor.execute("""
        ...
        SELECT r.EntityGUID, REPLACE(REPLACE(REPLACE(SUBSTRING(r.Remark, 1, 4000), CHAR(13)+CHAR(10), N'Â¶'), CHAR(13), N'Â¶'), CHAR(10), N'Â¶')
        FROM EntityRemark r
        ...
        for guid, remark in rows:
            if guid != current_guid:
                if current_guid is not None:
                    ...
                    merged_remarks = "; ".join(unique_remarks)[:4000].replace("\r\n", "Â¶").replace("\r", "Â¶").replace("\n", "Â¶")
