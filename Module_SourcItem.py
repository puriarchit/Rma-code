        ...
        if guid != current_guid:
            if current_guid is not None:
                ...
                if len(batch_to_insert) >= batch_size:
                    # Critical fix: setting size to 0 for SQL_WVARCHAR instructs pyodbc that this is a MAX type,
                    # which triggers parameter-at-execution streaming instead of trying to allocate a fixed buffer.
